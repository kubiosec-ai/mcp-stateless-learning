# SDK compatibility & backward compatibility
...
|---|---|---|---|
| `mcp` (official Python) | **2.0.0** | — | modern |
| `@modelcontextprotocol/server` / `client` (TS) | **2.0.0** | — | modern |
| `@modelcontextprotocol/sdk` (old TS package) | **1.30.0** | — | legacy |
| `fastmcp` (stable) | **3.4.5** | — | legacy-era default |
| `fastmcp` (beta, used here) | **4.0.0b1** | — | modern + legacy |
...
FastMCP 4 removes three context methods **from every protocol era**, so code
...
The replacement for multi-round interaction is returning an
`InputRequiredResult` and reading the answers on the next round, or calling
an LLM directly from the server.
